...
FILE_MARKER_DIR = "./data/file_markers_detection"
...
def main(
    ...
    for idx in tqdm(range(len(all_tuples))):
        ...
        expected_edf = os.path.basename(h5_fn.split('.h5')[0])
        print("Expected EDF filename:", expected_edf)

        # Build a dictionary mapping from EDF basenames to their full paths.
        edf_dict = {os.path.basename(file): file for file in edf_files}
        print("Available EDF files:", list(edf_dict.keys()))

        if expected_edf in edf_dict:
            edf_fn_full = edf_dict[expected_edf]
        else:
