...
def get_args():
    ...
    parser.add_argument('--combined_graph', action='store_true',
                        help="If set, use a precomputed (combined) adjacency graph for all samples.")

    parser.add_argument('--adj_mat_dir', type=str, default='./data/electrode_graph/adj_mx_3d.pkl',
                        help='Path to precomputed adjacency matrix file (only used if --combined_graph is set).')
    parser.add_argument('--normalization', type=str, default='per_clip_channel',
                        choices=['per_channel_dataset', 'global_dataset', 'per_clip_channel', 'per_clip_whole', 'none', None],
                        help='Normalization method to apply to EEG clips.')
