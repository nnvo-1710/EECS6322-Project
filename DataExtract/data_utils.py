...
def getSeizureTimes(file_name):
    ...
    """
    csv_file = file_name.split(".edf")[0] + ".csv_bi"  # Change file extension

    if not os.path.exists(csv_file):
        raise FileNotFoundError(f"Seizure event file {csv_file} not found.")
    ...
    with open(csv_file, "r") as f:
        for line in f:
            ...
            if label == "seiz":
                try:
                    start_time = float(parts[1])  # Second component
                    end_time = float(parts[2])    # Third component
                    seizure_times.append([start_time, end_time])
                except ValueError:
                    continue  # Skip lines with non-numeric time values
